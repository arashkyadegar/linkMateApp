...
const ShortLinkCreateForm = () => {
  ...
  const validateFields = () => {
    ...
    if (
      formData.customShortlink &&
      !/^[a-zA-Z0-9_-]+$/.test(formData.customShortlink)
    ) {
      newErrors.customShortlink =
        "Custom shortlink can only contain letters, numbers, hyphens, and underscores.";
    }
    setErrors(newErrors);
